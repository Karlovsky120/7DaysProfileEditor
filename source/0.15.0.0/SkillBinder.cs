...
namespace SevenDaysProfileEditor.Skills
{
    internal class SkillBinder
    {
        ...
        public SkillBinder(Skill skill, Value<int> playerLevel, List<string> unlockedSkillRecipeList)
        {
            this.skill = skill;
            ...
            this.unlockedRecipeList = unlockedSkillRecipeList;
        }
        ...
        public bool IsRecipeUnlocked(string recipeName)
        {
            ...
            {
                if (recipe.Key.Equals(recipeName) && recipe.Value <= level.Get())
                {
                    return true;
                }
            }
